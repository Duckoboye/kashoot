...
import KashootController from '../src/client/KashootController'
...
describe('Serial Logic Tests', () => {
  ...
  let kashootController: KashootController;
  ...
  before((done) => {
    ...
    kashootController = new KashootController(port, "http://localhost:5000")
  ...
  it('should be able to answer a question incorrectly', (done) => {
    port.port?.emitData('21\n')
    setTimeout(done, 200)
  })
  it('should be able to answer all questions', (done) => {
    port.port?.emitData('11\n')
    setTimeout(done, 200)
  })
